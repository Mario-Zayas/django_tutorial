        ...
        stage('Copiar settings') {
            ...
            stages {
			    ...
			    sh 'ssh -o StrictHostKeyChecking=no mario@maquinanodriza.mzgmaquina.es docker rmi -f mzaygar034/django_tutorial_jenkins:${env.BUILD_ID}
                            sh 'ssh -o StrictHostKeyChecking=no mario@maquinanodriza.mzgmaquina.es wget https://raw.githubusercontent.com/Mario-Zayas/django_tutorial/master/docker-compose.yaml -O docke -O docker-compose.yaml'
